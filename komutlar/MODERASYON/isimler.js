...
let listedData = data ? data.map((value, index) => `\`☆ ${value.Name} ${value.Age}\` (${value.Role.map(x => `${x ? `<@&${x}>` : "Rol Belirtilmemiş"}`) || "Cinsiyet Belirtilmemiş"})`) : "Bu Üyenin İsim Geçmişi Bulunamadı.";
