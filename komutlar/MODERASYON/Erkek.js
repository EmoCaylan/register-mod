  ...
  if (message.member.permissions.has(8) || message.member.roles.cache.some(e => arr.some(x => x == e))) {
      ...
      if (!user) return message.reply(`**Lütfen bir kişi etiketleyin veya ID yazınız** \`(örn: ${prefix[0]}erkek @Owsla Owsla 20)\``).then(x => x.delete({timeout: 5000}));
        ...
        if (user.user.username.includes(sunucutagi)) {
          ...
          if (otokayit) {
            await odb.set(`otokayit.${user.id}`, {Tag: sunucutagi, Name: isim, Age: yaş, Role: erkekRol})
          }
          await kdb.push(`isimler.${user.id}`, {Name: isim, Age: yaş, Role: erkekRol});
          ...
          if (otokayit) {
            await odb.set(`otokayit.${user.id}`, {Tag: sunucutagi2, Name: isim, Age: yaş, Role: erkekRol})
          }
          await kdb.push(`isimler.${user.id}`, {Name: isim, Age: yaş, Role: erkekRol});
          await fonksiyon(erkekRol, message, user, kızRol, kayitsizuyeRol);
        }
...
Toplamda ${data.length || 0} isim kayıtı bulundu. \n\n${data.length >=0 ? data.map((value, index) => `\`☆ ${value.Name} ${value.Age}\` (${value.Role.map(x => `${x ? `<@&${x}>` : "Rol Belirtilmemiş"}`)})`).join("\n") : "İsim Geçmişi Görüntülenemedi"}
`));
}
