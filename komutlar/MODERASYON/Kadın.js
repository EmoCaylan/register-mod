...
module.exports.run = async (client, message, args) => {
  ...
  if (message.member.permissions.has(8) || message.member.roles.cache.some(e => arr.some(x => x == e))) {
      ...
      if (!user) return message.reply(`**Lütfen bir kişi etiketleyin veya ID yazınız** \`(örn: ${prefix[0]}kadın @Owsla Owsla 20)\``).then(x => x.delete({timeout: 5000}));
        ...
        if (user.user.username.includes(sunucutagi)) {
          ...
          if (otokayit) {
            await odb.set(`otokayit.${user.id}`, {Tag: sunucutagi, Name: isim, Age: yaş, Role: kızRol})
          }
          await kdb.push(`isimler.${user.id}`, {Name: isim, Age: yaş, Role: kızRol});
          await fonksiyon(kızRol, message, user, kızRol, kayitsizuyeRol);
          ...
          if (otokayit) {
            await odb.set(`otokayit.${user.id}`, {Tag: sunucutagi2, Name: isim, Age: yaş, Role: kızRol})
          }
          await kdb.push(`isimler.${user.id}`, {Name: isim, Age: yaş, Role: kızRol});
          await fonksiyon(kızRol, message, user, kızRol, kayitsizuyeRol);
        }
